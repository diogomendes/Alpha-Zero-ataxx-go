...
class Go:
    ...
    def to_play(self): #Retorna quem é o proximo a jogar
        return 2 if self.turn == 1 else 1
    
    def legal_actions(self): #Movimentos possiveis
        legal = []
    ...
    def reset(self): #Faz um reset dos  valores iniciais
        #print(self.board) 
        self.possible=[]
        self.passe = 0
        self.board = np.zeros(shape=(tam, tam))
    ...
    def step(self, a): #Realiza uma jogada
        if a == tam*tam:
            self.passe += 1
    ...
    def get_observation(self): #retorna uma observação da jogada que vai ser usada pelo muzero para a rede neuronal
    ...
    def change(self): #Altera o turno de jogo
        if self.turn  == 1:
            return 2
        else:
            return 1
        
    def set_peca(self, coor): #Coloca uma peça no tabuleiro
        if self.turn==1:
            self.board[coor[0]][coor[1]] = 1
        else:
            self.board[coor[0]][coor[1]] = 2
    
    def jogada(self, coor): #Realiza uma jogada realizada pelo humano
        self.visited = [coor]
    ...
    def confirm_jog(self,board_new, coor):     #Confirma se é uma jogada possivel       
    ...
    def score(self): #Verifica o score no final do jogo
        self.territorio()
    ...
    def check_pecas(self): #Verifica se uma peça é do jogador atual
    ...
    def check_cap(self,coor):  #Verifica se uma peça foi capturada 
    ...
    def capture(self,stone): #Verifica as possiveis peças que podem ser capturadas
        stack=list()
        stack.append(stone)
    ...
    def legal_move(self,coor): #Verifica se o é possivel colocar uma peça naquele local
    ...
    def legal_move2(self,coor,board_n): #Verifica se o é possivel colocar uma peça naquele local
    ...
    def check_color(self,neigh): #Verifica se peças adjacentes são da mesma cor ou não
    ...
    def check_color2(self,neigh, board_n): #Verifica se peças adjacentes são da mesma cor ou não
    ...
    def pos_clear(self, coor): #Verifica se a posição tem alguma peça
        return self.board[coor[0]][coor[1]] == 0

    
    def expert_action(self):  #Jogada ramdom
        action = np.random.choice(self.legal_actions())
        return action
    
    def render(self): #Simples render do jogo para o terminal
        print(self.board)
    ...
    def load_model(self, checkpoint_path=None, replay_buffer_path=None):  #Faz load do modelo
        """
