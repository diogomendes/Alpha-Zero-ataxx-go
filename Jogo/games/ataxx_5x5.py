...
from MCTS_Player import MonteCarloTreeSearch
...
class Ataxx:
    ...
    def calc_reward(self):
        ...
        if(self.player==-1):  
            ...
            return(((jog2-jog1)/(jog2+jog1)))   
        else:
            if(self.have_winner()):
                return (2*(jog1-jog2))
            
            return(((jog1-jog2)/(jog2+jog1)))
        
        
        
        
    def check_draw(self,jog1,jog2):
        for i in range(self.row):
            for j in range(self.column):
                if(self.board[i][j]==0):
                    return False
    ...
    def have_winner(self):
        ...
        for row in range(self.row):
            ...
            if (isThereTangentPositionsRed or isThereMovementPositionsRed) and (isThereMovementPositionsBlue or isThereTangentPositionsBlue):
                break
        v = self.GetBoardValue()
        # if v > 0:
        #     print("Player one is winning")
        # elif v < 0:
