...
class GO:
    ...
    def game_geral(self, event): #Evento principal do jogo, quando clique na parte grafica verifica se pode jogar na posiçao clicada e se possivel joga e depois do human jogar, joga o computador
        global passe
        global turn
        ...
        #opponent = "human"
        muzero_player = 1 #self.config.muzero_player #ver isto 
        ...
        if (not done and len(self.game_history.action_history) <= self.config.max_moves):
                '''
                assert (
                ...
                if event.x < 600 and event.y < 560 and event.x >=40 and event.y >= 15:
                    pixels_position = [event.x, event.y]
                    coords_pos = self.convert_pixels_to_coords(pixels_position)
                    action = coords_pos[0]*tam+coords_pos[1]
                    if self.pos_clear(coords_pos):
                        t=self.jogada(coords_pos)
                        self.desenhar_whiteblack()
                        if(t):
                            ...
                            #Jogada do humano para o muzero
    ...
    def passar_turno2(): #Botao para passar o turno
        global turn, passe
        turn = go.change(turn)
        passe +=1
        if passe ==2:
